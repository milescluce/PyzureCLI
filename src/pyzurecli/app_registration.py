...
class AzureCLIAppRegistration:
    ...
    def __init__(self, azure_cli: az, **kwargs):
        for kwarg in kwargs:
    ...
    def creds(self) -> AppRegistrationCreds:
        ...
        result = user_image.run(
            ...
            f"--web-redirect-uris {redirect_uri} "
            f"--public-client-redirect-uris {redirect_uri} "
        ...
        if "Found an existing application" in result.output: log.success(f"{self}: Successfully patched multi-tenant app: {app_data['appId']}")
        else: log.success(f"{self}: Successfully created multi-tenant app: {app_data['appId']}")
        ...
        if app: user_image.run(
                    (f"az ad app update --id {app.appId} "
                    "--sign-in-audience AzureADMultipleOrgs "
                    "--is-fallback-public-client true"),
                    headless=True
                )
        else: raise RuntimeError(f"Error in creating Multi-Tenant App!")
    ...
    def admin_consent_url(self) -> str:
        ...
        return (
            ...
            f"redirect_uri={self.azure_cli.redirect_uri}"
            f"scope={self.scopes}"
        )
