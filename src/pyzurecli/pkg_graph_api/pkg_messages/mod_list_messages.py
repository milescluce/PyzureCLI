...
async def list_conversations_with_person(self: _GraphAPIMethods, person: str,
    ...
    def process_group(messages, is_from_you: bool):
        ...
        for message in messages:
            if not (conversation_id := message.pop("conversationId")): raise KeyError(
                f"Missing key, 'conversationId' in {message}")
    ...
    conversations = [
        (conv_id, earliest_messages[conv_id][0], earliest_messages[conv_id][1], earliest_messages[conv_id][2]) for
        conv_id in conversations]

    num_of_conversations = len(conversations)
    num_of_messages = len(messages_from) + len(messages_to)
    average_messages = (num_of_messages / num_of_conversations)
    log.debug(
